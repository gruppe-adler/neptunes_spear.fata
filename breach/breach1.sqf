	...
	[] spawn {
		...
		while {true} do {
			...
			waitUntil {
				...
				(_isAdjust && {_isNext or {_isPrev}}) or (inputAction "User1" > 0) // this allows for user to hit ctrl and scroll with mouse wheel or to hit there custom controls user action 1 key
				};
		...
		if (action123 < 1) then
			{
				...
				//[] execvm "breach\breach2.sqf";
				[] execvm "breach\breach3.sqf";
				//[] execvm "breach\breach2.sqf"; // excute breach2 script which creates charge
				...
				//_doorPhase = cursorTarget1 animationPhase selectionName;
